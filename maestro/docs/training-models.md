...
Make sure all of the dependencies for training models are installed and built. You can do so by following the instructions in [Building](building.md)—make sure you do *not* use the `--minimal` flag or `arqon-maestro-minimal` Docker image (which are used only for running Arqon Maestro).
...
- Finds for the generated data in `$ARQON_MAESTRO_LIBRARY_ROOT/code-engine-training/data/{MODEL_TYPE}/{LANGUAGE}`
