...
ensure_parakeet_sidecar_ready() {
  ...
  if curl -fsS "http://127.0.0.1:5001/ready" >/dev/null 2>&1; then
    print_green "✅ [ArqonMaestro] Parakeet sidecar READY at http://127.0.0.1:5001/ready (local endpoint preflight passed)"
    return 0
  fi
  local health_payload
  health_payload="$(curl -sS "http://127.0.0.1:5001/health" 2>/dev/null || true)"

  print_red "============================================================"
  print_red "FATAL: Parakeet sidecar is NOT ready (127.0.0.1:5001/ready failed)"
  print_red "Refusing to start ArqonMaestro with local endpoint."
  print_red "============================================================"
  if [[ -n "${health_payload}" ]]; then
    print_yellow "Sidecar /health: ${health_payload}"
    print_yellow "Tip: if model_loaded=false with a cuDNN mismatch, unset LD_LIBRARY_PATH before launching the sidecar."
  fi
  print_red ""
