...
import arqon_maestro.repositories.github
...
@click.option("--url", default=f"{arqon_maestro.config.base_url}/repositories")
def create_small_repository(languages, url):
    """Create a small repository of sampled files"""
    for language in languages:
        arqon_maestro.repositories.create_small_repository(language, url)
...
def crawl(languages, limit, page_count, page_size, rounds):
    ...
    for language in languages:
        arqon_maestro.repositories.github.crawl(
            language,
            rounds,
            page_count,
            page_size,
...
def download(languages, small_repository, url):
    ...
    for language in languages:
        arqon_maestro.repositories.download(language, url, small_repository)
