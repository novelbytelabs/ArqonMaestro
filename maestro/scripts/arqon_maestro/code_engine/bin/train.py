...
def _run_marian(data_dir, model_dir, model, num_batches, gpus):
    ...
    os.chdir(arqon_maestro.config.library_path("marian"))
    ...
    --valid-script-path {arqon_maestro.config.library_path("marian", "scripts", "validate.sh")} \
...
def train_model(model, language, gpus, test_mode):
    ...
    subprocess.check_call(
        f"cat {data_path}/train.unk.input {data_path}/train.unk.output | {arqon_maestro.config.library_path('marian', 'build', 'marian-vocab')} -m 5500 > {prefix}/vocab.yml",
        shell=True,
    )
    if test_mode:
        num_batches = 5000
