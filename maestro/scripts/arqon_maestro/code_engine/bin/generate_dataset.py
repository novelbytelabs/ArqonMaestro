...
import arqon_maestro.code_engine
...
def _run_corpusgen(args, optional_pipe_in=None):
    ...
    corpusgen_command = f"{arqon_maestro.config.source_path('corpusgen', 'build', 'install', 'corpusgen', 'bin', 'corpusgen')} {args}"
...
def _generate_dataset(prefix, language, model, vocab_size, test_mode):
    ...
    arqon_maestro.repositories.generate_file_list(language, count, prefix, test_mode)
...
def generate_dataset(model, language, test_mode, url):
    ...
    os.chdir(arqon_maestro.config.source_path())
