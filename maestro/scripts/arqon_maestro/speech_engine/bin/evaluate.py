...
def evaluate_model():
    ...
    output_path = os.path.join(arqon_maestro.config.library_path("evaluation"), output_name)
    os.makedirs(output_path, exist_ok=True)
    serenade.packaging.models.download_speech_engine(model)

    replayer_path = arqon_maestro.config.source_path(
        "scripts", "replayer", "build", "install", "replayer", "bin", "replayer"
    )

    subprocess.check_call(
        f"""{replayer_path} \
            --audio "{arqon_maestro.config.library_path("audio")}" \
            --output "{output_path}" \
            --test-set {arqon_maestro.config.library_path("test-set.jsonl")}
        """,
        shell=True,
        env={
            **os.environ,
