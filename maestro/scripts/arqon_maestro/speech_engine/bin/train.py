...
sys.path.append(
    os.path.join(os.getenv("SERENADE_SOURCE_ROOT") or os.path.expanduser("~/maestro"), "scripts")
)


import arqon_maestro.config
import arqon_maestro.repositories
import arqon_maestro.speech_engine
...
def _apply_g2p(words):
    g2p_model = arqon_maestro.config.library_path("lm", "tmp", "g2p-model")
    missing_lexicon = arqon_maestro.config.library_path("lm", "tmp", "missing-lexicon")
    missing_words = arqon_maestro.config.library_path("lm", "tmp", "missing.txt")
    ...
    subprocess.check_call(
        f'cp {arqon_maestro.config.library_path("models", "speech-engine", "g2p", "model.fst")} {os.path.join(g2p_model, "model.fst")} && \
        bash -c "source {arqon_maestro.config.library_path("kaldi", "tools", "env.sh")} && \
        rm -rf {missing_lexicon} && \
        cd {arqon_maestro.config.library_path("kaldi", "egs", "wsj", "s5")} && \
...
def _create_const_arpa(lm, output):
    ...
    subprocess.check_call(
        [
            ...
            arqon_maestro.config.library_path("kaldi"),
            lm,
...
def _finalize_corpus(languages):
    with open(f"{arqon_maestro.config.library_path('lm', 'corpus.txt')}", "w") as output:
...
def _generate_corpus(language, sample_size=None):
    ...
    subprocess.check_call(
        ...
        {arqon_maestro.config.source_path('corpusgen', 'build', 'install', 'corpusgen', 'bin', 'corpusgen')} \
...
def _generate_kaldi_graph(lm, lexicon, acoustic_model):
    ...
    subprocess.check_call(
        [
            ...
            lm,
            lexicon,
            arqon_maestro.config.source_path("scripts", "serenade", "speech_engine", "training"),
            acoustic_model,
        ]
    )
...
def _generate_lexicon(languages):
    ...
    hand_labeled_lexicon = [
        ...
        for s in subprocess.check_output(
            f"cat {arqon_maestro.config.source_path('scripts', 'serenade', 'speech_engine', 'lexicon', '*')}",
            shell=True,
        )
    ...
    words = set(s[0] for s in lexicon)
    vocab = _vocab(arqon_maestro.config.library_path("lm", "corpus.txt"), words)
...
def _generate_ngram_model(corpus, output, vocab=None, **kwargs):
    ...
    subprocess.check_call(
        [
            arqon_maestro.config.library_path(
...
def _language_path(language):
    return arqon_maestro.config.library_path("lm", f"{language}-data")
...
def _mix_ngram_models(base_model, mix_model, output, **kwargs):
    ...
    subprocess.check_call(
        [
            arqon_maestro.config.library_path("kaldi", "tools", "srilm", "bin", "i686-m64", "ngram"),
            *[
                str(arg)
                for arg in [
                    "-order",
...
def _require_srilm():
    if not os.path.exists(arqon_maestro.config.library_path("kaldi", "tools", "srilm", "bin")):
        print("You need to install SRILM by running kaldi/tools/install_srilm.sh")
        sys.exit(1)


def _spelled_words_lexicon():
    spelling_path = arqon_maestro.config.source_path("scripts", "serenade", "speech_engine", "spelling")
...
def generate_dataset(test_mode):
    ...
    path = arqon_maestro.config.library_path("lm")
    shutil.rmtree(path)
    os.makedirs(path)
    ...
    for language in config:
        ...
        arqon_maestro.repositories.generate_file_list(
            language,
...
@click.option(
    ...
    default=f"{arqon_maestro.config.base_url}/repositories",
)
def train_model(test_mode, url):
    ...
    for language in list(config.keys()):
        arqon_maestro.repositories.download(language, url, test_mode)

    _generate_lexicon(list(config.keys()))

    _filter_vocabulary(
        arqon_maestro.config.library_path("lm", "corpus.txt"),
    ...
    _generate_kaldi_graph(
        ...
        arqon_maestro.config.library_path("models", "speech-engine", "acoustic-model"),
    )
