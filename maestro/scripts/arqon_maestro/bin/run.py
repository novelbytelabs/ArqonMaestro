...
@click.option(
    "--config",
    default=arqon_maestro.config.source_path("config", "services.yaml"),
...
def main(build, commands, config, environment, log, run_services, services, tests):
    """Run Arqon Maestro services and commands"""

    global command_subprocesses, service_subprocesses

    if build:
        subprocess.check_call(
            f"cd {arqon_maestro.config.source_path()} && gradle installd",
            shell=True,
        )
    ...
    env = {**os.environ, **arqon_maestro.config.services(environment)}
    if tests:
        log = "error"
        commands += tests
    ...
    if run_services and services:
        git_directory = arqon_maestro.config.source_path(".git")
        version_file = arqon_maestro.config.source_path("VERSION")
        env["LOG_LEVEL"] = log
        ...
        for service in services:
            ...
            if service == "code-engine":
                service_subprocesses.append(
                    subprocess.Popen(
                        "code-engine/server/build/code-engine/arqon-maestro-code-engine",
                        env=service_env,
                        shell=True,
                    )
                )
                ...
                service_subprocesses.append(
                    subprocess.Popen(
                        "speech-engine/server/build/speech-engine/arqon-maestro-speech-engine",
                        env=service_env,
                        shell=True,
                    )
                )
