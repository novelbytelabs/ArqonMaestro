...
describe("ChunkManager H3 workflow candidate scoring evidence", () => {
  ...
  function makeBareManager(): any {
    ...
    manager.relativeChunkNowMs = () => 555;
  ...
  it("emits candidate scoring fields for an exact emerged family", () => {
    ...
    const emit = (chunkId: string, semanticAddressId: string) => {
      ...
      ChunkManager.prototype.emitH3Evidence.call(manager, chunkId, "voice_semantic_address_lookup_completed", {
        ...
        reason: `score_${semanticAddressId}`,
      });
    };
    ...
    const lastCall: any = (evidenceSpy.mock.calls[evidenceSpy.mock.calls.length - 1] as any)[0];
    ...
    expect(lastCall.workflowCandidateConfidenceScore).toBeGreaterThanOrEqual(70);
    expect(lastCall.workflowCandidateCreationRiskScore).toBeLessThanOrEqual(20);
    expect(lastCall.workflowCandidateScoringReasonCodes).toContain(
      "workflow_candidate_scoring_exact_family_shape"
    );
  });

  it("emits elevated abstraction risk when a second unstable family variant emerges", () => {
    const { ChunkManager, manager, h23Recorder, runtimeEvidence } = makeBareManager();
    jest.spyOn(h23Recorder, "getTraceSnapshot").mockReturnValue([]);
    const decisionSpy = jest.spyOn(h23Recorder, "getLatestDecision");
    const evidenceSpy = jest.spyOn(runtimeEvidence, "emitH3RuntimeEvidence").mockImplementation((event: any) => event);
    ...
    const emit = (chunkId: string, semanticAddressId: string) => {
      ...
      ChunkManager.prototype.emitH3Evidence.call(manager, chunkId, "voice_semantic_address_lookup_completed", {
        ...
        canonicalMergedText: semanticAddressId,
        transcriptText: semanticAddressId,
        reason: `score_variant_${semanticAddressId}`,
      });
    };
    ...
    expect(lastCall).toEqual(expect.objectContaining({
      ...
      workflowCandidateScoringSchemaVersion: "3j_workflow_candidate_scoring_v1",
      workflowCandidateScoringEligible: true,
      workflowCandidateCreationRiskBand: "moderate",
    }));
    expect(lastCall.workflowCandidateAbstractionRiskComponent).toBeGreaterThanOrEqual(50);
    expect(lastCall.workflowCandidateScoringReasonCodes).toContain(
