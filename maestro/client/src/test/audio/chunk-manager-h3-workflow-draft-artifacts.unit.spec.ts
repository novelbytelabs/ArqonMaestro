...
describe("ChunkManager H3 workflow draft artifact evidence", () => {
  ...
  function makeBareManager(): any {
    ...
    manager.relativeChunkNowMs = () => 999;
    for (const [id, regionId] of [["chunk-1", "open"], ["chunk-2", "mid"]] as const) {
  ...
  it("emits draft and library API preview fields for an exact stable family", () => {
    ...
    const emit = (chunkId: string, semanticAddressId: string) => {
      ...
      ChunkManager.prototype.emitH3Evidence.call(manager, chunkId, "voice_semantic_address_lookup_completed", {
        ...
        reason: `draft_${semanticAddressId}`,
      });
    };
    ...
    expect(lastCall).toEqual(expect.objectContaining({
      ...
      workflowLibraryApiSchemaVersion: "3j_workflow_draft_artifacts_v1",
    ...
    expect(["suggested", "queued_for_review"]).toContain(lastCall.workflowDraftArtifactReviewState);
    expect(["review_candidate", "auto_created_draft_candidate", "persistent_draft_candidate"]).toContain(
      lastCall.workflowLibraryApiCandidateState
    );
  });
});
