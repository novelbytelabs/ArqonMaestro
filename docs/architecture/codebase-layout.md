...
```mermaid
flowchart TB
  Client[Electron Client] --> Core[core]
  Core --> Speech[Speech Engine]
  Core --> Code[Code Engine]
  Core --> Plugins[Plugins and Editor Integrations]
  Models[Training and Model Artifacts] --> Speech
  Models --> Code
```

## Why This Matters

This layout is useful when deciding where a bug belongs, where a new feature should live, and which subsystem owns a given runtime behavior.
...
`corpusgen` is the offline service that generates the synthetic data used to train auto-style models, transcript-parser models, contextual language models, and speech language models. More details can be found [here](../models/generating-data.md).

### code-engine training scripts

The code engine training scripts are used to train auto-style models, contextual language models, and transcript-parser models. Instructions for training these models can be found [here](../models/training-models.md), and details about the model architecture can be found [here](../models/model-architecture.md).

### speech-engine training scripts

The speech engine training scripts are used to train Kaldi language models. Instructions for training these models are [here](../models/training-models.md), and details about the speech engine system are [here](../models/model-architecture.md).
